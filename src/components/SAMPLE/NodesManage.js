...
class NodesManage {
    ...
    connect(from, to) {
        ...
        if (from.refreshRefers !== undefined) from.refreshRefers(to == 'MOUSE')
        if (to.refreshSources !== undefined) to.refreshSources(to == 'MOUSE')
    ...
    // Graph Calculations
    calculateAllDepths () {
        for (let node of this.getAllNodes()) node.calculateDepth()
    }
    // Storage Management
