...
class Node {
    constructor (nodeManage, initialProps) {
        ...
        this.props = {
            ...
            sources: {
                label: 'Direct Sources Count',
                value: 0
            },
            refers: {
                label: 'Direct Refers Count',
                value: 0
            },
            depth: {
                label: 'Node Depth',
    ...
    setProp (prop, value, pushToPanel = true) {
        if (pushToPanel && this.nodeManage !== 'GHOST') this.nodeManage.panel.onPropSet(this, prop, value)
    ...
    refreshRefers (noNeedToUpdateEverything = false) {
        if (this.nodeManage !== 'GHOST') {
            this.rfrs = this.nodeManage.getArrowByFrom(this)
            this.setPropertyRefers(this.rfrs.length)
            if (!noNeedToUpdateEverything) this.nodeManage.calculateAllDepths()
        }
    }
    refreshSources (noNeedToUpdateEverything = false) {
        if (this.nodeManage !== 'GHOST') {
            this.srcs = this.nodeManage.getArrowByTo(this)
            this.setPropertySources(this.srcs.length)
            if (!noNeedToUpdateEverything) this.nodeManage.calculateAllDepths()
        }
    }
    calculateDepth () {
        this.depthNodes = this.getAllDepthNodes()
        this.setPropertyDepth(this.depthNodes == Infinity ? Infinity : this.depthNodes.length)
    }
    getAllDepthNodes (startNode) {
        let depthNodes = []
        const isStartpoint = startNode == undefined
        startNode = startNode || this
        for (let src of this.srcs) {
            let node = src.from
            if ((!isStartpoint && startNode == node) || node.depthNodes == Infinity) return Infinity // We have a cycle!
            if (node.srcs.length > 0) {
                const BottomLevelsDepthNodes = node.getAllDepthNodes(startNode)
                if (BottomLevelsDepthNodes == Infinity) return Infinity
                depthNodes = depthNodes.concat(BottomLevelsDepthNodes)
            }
    ...
    setPropertySources (value) {
        this.setProp('sources', value)
    }
    setPropertyRefers (value) {
        this.setProp('refers', value)
    }
    setPropertyDepth (value) {
        this.setProp('depth', value)
    }
    render () {
